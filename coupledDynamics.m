...
% See if we need to calculate the Junction Point Dynamics
calc_Junction = (norm(Pi_c) + norm(Pj_t)) > 0;
% See if we need to calculate the Attitude Dynamics
...
dx = [x0_rho_ij_d;
    rho_ij_dd + u_pos ;  % Add Acceleration Due to Thrust
    dx_att;
    h_wcd];
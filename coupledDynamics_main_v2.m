...
far_Rend = false;
unCoupled = true;  % Only Relevant when doing Close Range
...
if far_Rend
    ...
    %omega0      = [-10.3;0.5;6.1]*1e-2;  % Rad/sec
    ...
    omega0      = [-0.4;0.5;0.2]*1e-0;  % Rad/sec
...
state_hist_SDRE = deval(relative_trajectory_SDRE, tspan);
 ...
 relative_trajectory_LQR = ode45(@(t,x) coupledDynamics(t, x, mu,a_t,e_t,'LQR',B,R,Q,I_c,P1_c,P0_t), tspan, x0, options);
...
q_hist = state_hist_SDRE(7:10,:)';
figure;
ylim_array = [-10 5;-5 10;-1 2]*1e-3;
axis_str = 'xyz';
for ii = 1:3
    subplot(3,1,ii)
    plot(tspan,omega_hist(:,ii),'LineWidth', 1.5);grid on
...
for ii = 1:3
    subplot(3,1,ii)
    plot(tspan,h_wc_hist(:,ii), 'LineWidth', 1.5);grid on
...
for ii = 1:3
    ...
    subplot(3,1,ii)
        hold on
    plot(tspan,q_hist(:,ii),'LineWidth', 1.5);grid on
   % plot(tspan,q_hist_coupled(:,ii), 'r--','LineWidth', 1.5)
...
if 1
     %relative position
    figure()
    %sgtitle('Relative Position Time History','Interpreter','latex')
    subplot(3,1,1)
    hold on
    plot(tspan, state_hist_SDRE(1,:), 'r','LineWidth', 1.5)
    plot(tspan, state_hist_SDRE_coupled(1,:), 'b--','LineWidth', 1.5)
    %plot(tspan, state_hist_LQR(1,:), 'b--','LineWidth', 1.5)
    xlabel('time [sec]','Interpreter','latex')
    %ylabel('$\Delta X(t) \; [m]$','Interpreter','latex')
    ylabel('$\rho_x (t) \; [m]$','Interpreter','latex')
    ...
    plot(tspan, state_hist_SDRE(2,:), 'r','LineWidth', 1.5)
        plot(tspan, state_hist_SDRE_coupled(2,:), 'b--','LineWidth', 1.5)
    %plot(tspan, state_hist_LQR(2,:), 'b--','LineWidth', 1.5)
    xlabel('time [sec]','Interpreter','latex')
    ylabel('$\rho_y (t) \; [m]$','Interpreter','latex')
    ...
    plot(tspan,state_hist_SDRE(3,:), 'r','LineWidth', 1.5)
        plot(tspan, state_hist_SDRE_coupled(3,:), 'b--','LineWidth', 1.5)
   % plot(tspan, state_hist_LQR(3,:), 'b--','LineWidth', 1.5)
    ...
    subplot(3,1,1)
    hold on
    %sgtitle('Relative Velocity Time History','Interpreter','latex')
    plot(tspan, state_hist_SDRE(4,:), 'r','LineWidth', 1.5)
    plot(tspan, state_hist_SDRE_coupled(4,:), 'b--','LineWidth', 1.5)
    %plot(tspan, state_hist_LQR(4,:), 'b--','LineWidth', 1.5)
    xlabel('time [sec]','Interpreter','latex')
    ylabel('$\dot{\rho}_y (t) \; [m/s]$','Interpreter','latex')
    set(gca,'TickLabelInterpreter','latex')
        %legend('SDRE', 'LQR')
    grid on
    ...
    plot(tspan, state_hist_SDRE_coupled(5,:), 'b--','LineWidth', 1.5)
    %plot(tspan, state_hist_LQR(5,:), 'b--','LineWidth', 1.5)
    xlabel('time [sec]','Interpreter','latex')
    %ylabel('$\Delta v_y (t)  \; [m/s]$','Interpreter','latex')
    ylabel('$\dot{\rho}_z (t)   \; [m/s]$','Interpreter','latex')
    set(gca,'TickLabelInterpreter','latex')
    grid on
    ...
    plot(tspan, state_hist_SDRE_coupled(6,:), 'b--','LineWidth', 1.5)
     %plot(tspan, state_hist_LQR(6,:), 'b--','LineWidth', 1.5)
    xlabel('time [sec]','Interpreter','latex')
    ylabel('$\Delta v_z (t)  \; [m/s]$','Interpreter','latex')
    set(gca,'TickLabelInterpreter','latex')
    grid on

    %% Control history

    X_SDRE = state_hist_SDRE(1:6,:);
    X_LQR = state_hist_LQR(1:6,:);
    total_u_effort_SDRE = 0;
    total_u_effort_LQR = 0;

    for tt = 1:numel(tspan)

        t = tspan(tt);
        K_SDRE = calculateControllerGainfunction(t, X_SDRE(:,tt),  mu, a_t, e_t, 'SDRE',B,R,Q);
        K_LQR = calculateControllerGainfunction(t, X_LQR(:,tt),  mu, a_t, e_t, 'LQR',B,R,Q);
        u_history_global_SDRE(:,tt) = K_SDRE*X_SDRE(1:6,tt);
        u_history_global_LQR(:,tt) = K_LQR*X_LQR(:,tt);

        total_u_effort_SDRE = total_u_effort_SDRE + norm(u_history_global_SDRE(:,tt),2);
        total_u_effort_LQR = total_u_effort_LQR + norm(u_history_global_LQR(:,tt),2);
    end
...
total_u_effort_LQR
    ...
    xlabel('time [sec]','Interpreter','latex')
    ylabel('$u_x (t)$','Interpreter','latex')
     %legend('SDRE', 'LQR')
    grid on
    subplot(3,1,2)
    hold on
   ...
   % plot(t_history_global, u_history_global_LQR(2,:), 'b--','LineWidth', 1.5)
    xlabel('time [sec]','Interpreter','latex')
    ylabel('$u_y (t)$','Interpreter','latex')
    grid on
    subplot(3,1,3)
    hold on
    ...
    xlabel('time [sec]','Interpreter','latex')
    ylabel('$u_z (t)$','Interpreter','latex')
    grid on
